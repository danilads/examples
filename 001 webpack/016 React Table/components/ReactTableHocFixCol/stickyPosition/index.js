    ...
    getColumnsWithFixed(columns, parentIsfixed, parentIsLastFixed, parentIsFirstFixed) {
      return columns.map((column, index) => {
        ...
        const output = {
          ...column,
          fixed,
          className: (`${column.className?column.className:''} ${fixed ?'rthfc-td-fixed':''} ${columnIsLeftFixed ? 'rthfc-td-fixed-left':''} ${columnIsRightFixed ? 'rthfc-td-fixed-right':''} ${isLastFixed ? 'rthfc-td-fixed-left-last' : ''} ${isFirstFixed ? 'rthfc-td-fixed-right-first':''}`),
          style: {
            ...column.style,
            left,
            right,
    ...
    render() {
      ...
      return (
        <ReactTable
          {...props}
          ref={innerRef}
          className={(`${className?className:''} ${this.uniqClassName?this.uniqClassName:''} rthfc -sp`)}
