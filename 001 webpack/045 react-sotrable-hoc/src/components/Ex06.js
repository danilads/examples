...
const VirtualList = ({loadData, addRemoveCollpaseId, items, getRef}) => {
  ...
  const renderRow = ({
    ...
    if (index === 0) {
      return <Card style={style} key={key} index={index} title={title} onClick={() => addRemoveCollpaseId(title)} isHeader={isHeader} />
    }
    return <SortableItem style={style} key={key} index={index} title={title} isHeader={isHeader} onClick={() => isHeader && addRemoveCollpaseId(title)} />;
  };
...
export const Ex06 = () => {
  ...
  return (
    <div className='root'>
      <h1>EX 06 sortable</h1>
      <h1>PREV CASES: virualized 🥷 headers (fixed) 🥷 loading with scroll 🥷 collapse groups </h1>
      <h1>CURRENT CASE: sorting(reRequestLoaded)</h1>
