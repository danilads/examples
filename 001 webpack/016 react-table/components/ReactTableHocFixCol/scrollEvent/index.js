...
export default (ReactTable) => {
  class ReactTableFixedColumns extends React.Component {
    ...
    constructor(props) {
      ...
      this.uniqClassName = this.props.uniqClassName || (`rthfc-${Math.random().toString().slice(2)}`);
    ...
    getColumnsWithFixed = (columns, parentIsfixed, parentIsLastFixed, parentIsFirstFixed) => columns.map((column, index) => {
      ...
      const output = {
        ...column,
        fixed,
        className: (`${column.className?column.className:''} ${fixed?'rthfc-td-fixed':''} ${isLeftFixed({ fixed })?'rthfc-td-fixed-left':''} ${ isRightFixed({ fixed }) ? 'rthfc-td-fixed-right':''} ${isRightFixed({ fixed }) ?'rthfc-td-fixed-right':''} ${isLastFixed ? 'rthfc-td-fixed-left-last':''} ${isFirstFixed ? 'rthfc-td-fixed-right-first':''}`),
        headerClassName: (`${column.headerClassName?column.headerClassName:''} ${fixed ? 'rthfc-th-fixed':''} ${fixed ? 'rthfc-th-fixed': ''} ${isLeftFixed({ fixed }) ? 'rthfc-th-fixed-left':''} ${isRightFixed({ fixed }) ? 'rthfc-th-fixed-right':''} ${(_parentIsLastFixed || (parentIsLastFixed && isLastFixed)) ? 'rthfc-th-fixed-left-last':''} ${  (_parentIsFirstFixed || (parentIsFirstFixed && isFirstFixed)) ? 'rthfc-th-fixed-right-first':''}`),
      };
    ...
    render() {
      ...
      return (
        <ReactTable
          {...props}
          ref={innerRef}
          className={(`${className?className:''} rthfc -se ${this.uniqClassName?this.uniqClassName:''}`)}
